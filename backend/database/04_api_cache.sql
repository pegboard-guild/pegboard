-- 04_api_cache.sql
-- API-first caching system per ChatGPT5 Pro's recommendation

-- Add ingest_mode to data_partitions first (before any views)
ALTER TABLE data_partitions
  ADD COLUMN IF NOT EXISTS ingest_mode TEXT
  CHECK (ingest_mode IN ('api','bulk')) DEFAULT 'api';

-- Set initial ingest modes per ChatGPT5 Pro's recommendation
UPDATE data_partitions
SET ingest_mode = CASE
  WHEN id LIKE 'theunitedstates:%' THEN 'bulk'   -- tiny JSON, safe to bulk once
  ELSE 'api'                                      -- everything else API-first
END;

-- Main cache table
CREATE TABLE IF NOT EXISTS api_cache (
  key TEXT PRIMARY KEY,                -- stable hash of (source, url, params)
  source TEXT NOT NULL,                -- 'openstates', 'nyol', 'govinfo', 'congress', 'fec', etc.
  url TEXT NOT NULL,
  params JSONB,                        -- normalized request params
  status INTEGER,                      -- HTTP status
  body JSONB,                          -- response payload (or NULL on error)
  etag TEXT,
...
CREATE INDEX IF NOT EXISTS idx_api_cache_access ON api_cache(last_accessed DESC);

-- Simple eviction: remove expired or LRU beyond a soft cap
...
BEGIN
  ...
  -- LRU trim if still above soft cap
...
-- Cache hit rate monitoring (last 24h)
CREATE OR REPLACE VIEW api_cache_hit_rate AS
SELECT
  source,
  COUNT(*) FILTER (WHERE fetched_at < NOW() - INTERVAL '5 minutes'  -- heuristic: reused entries
...
-- Freshness for TX sessions (based on tx_bills.updated_at)
CREATE OR REPLACE VIEW tx_freshness AS
SELECT
  session,
...
-- Function to get or fetch cached data
...
-- Function to store API response in cache
...
-- TTL configuration table for different sources/endpoints
CREATE TABLE IF NOT EXISTS cache_ttl_config (
  source TEXT NOT NULL,
  endpoint_pattern TEXT NOT NULL,  -- regex pattern for URL matching
  ttl_seconds INTEGER NOT NULL,
  description TEXT,
  PRIMARY KEY (source, endpoint_pattern)
);

-- Insert default TTL configurations per ChatGPT5 Pro's guidelines
...
-- Function to get TTL for a given source and URL
...
BEGIN
  ...
  ORDER BY length(endpoint_pattern) DESC  -- Most specific pattern first
  LIMIT 1;
  ...
  RETURN COALESCE(v_ttl, 3600);  -- Ultimate fallback: 1 hour
END;
$$ LANGUAGE plpgsql;

-- Scheduled job to purge cache (run daily)
-- Note: This would be set up as a Supabase cron job
-- SELECT cron.schedule('purge-api-cache', '0 3 * * *', 'SELECT purge_api_cache();');