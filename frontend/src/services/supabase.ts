...
export const getTrendingBills = async (
  ...
  // Get top bills
  const sortedByCount = Object.entries(pegCounts)
    .sort(([, a], [, b]) => (b as number) - (a as number));

  const topBillIds = sortedByCount
  ...
  // Preserve original order defined by topBillIds
  const orderIndex: Record<string, number> = topBillIds.reduce((acc: Record<string, number>, id: string, idx: number) => {
    acc[id] = idx; return acc;
  }, {} as Record<string, number>);
  ...
  return bills
    ...
    .sort((a: any, b: any) => (orderIndex[a.bill_id] ?? 0) - (orderIndex[b.bill_id] ?? 0));
};

// Get up/down/comment counts for any target
export const getPegCounters = async (
  targetType: 'bill' | 'member' | 'vote',
  targetId: string,
  zipcode?: string
): Promise<{ up: number; down: number; comments: number; upLocal?: number; downLocal?: number; commentsLocal?: number }> => {
  if (!supabase) {
    return { up: 0, down: 0, comments: 0 };
  }
  ...
  const up = (all || []).filter((p: any) => p.sentiment === 'approve').length;
  const down = (all || []).filter((p: any) => p.sentiment === 'disapprove').length;
  const comments = (all || []).filter((p: any) => p.comment && p.comment.trim().length > 0).length;

  if (!zipcode) return { up, down, comments };
  ...
  const upLocal = (local || []).filter((p: any) => p.sentiment === 'approve').length;
  const downLocal = (local || []).filter((p: any) => p.sentiment === 'disapprove').length;
  const commentsLocal = (local || []).filter((p: any) => p.comment && p.comment.trim().length > 0).length;

  return { up, down, comments, upLocal, downLocal, commentsLocal };
};

// Batch version to minimize many per-card queries
export const getPegCountersBatch = async (
  targetType: 'bill' | 'member' | 'vote',
  targetIds: string[],
  zipcode?: string
): Promise<Record<string, { up: number; down: number; comments: number }>> => {
  if (!supabase || targetIds.length === 0) {
    return {} as Record<string, { up: number; down: number; comments: number }>;
  }

  const { data } = await supabase
    .from('pegs')
    .select('target_id, sentiment, comment')
    .eq('target_type', targetType)
    .in('target_id', targetIds);

  const result: Record<string, { up: number; down: number; comments: number }> = {};
  for (const id of targetIds) {
    result[id] = { up: 0, down: 0, comments: 0 };
  }

  for (const row of (data || []) as Array<{ target_id: string; sentiment?: string; comment?: string }>) {
    const id = row.target_id;
    if (!result[id]) result[id] = { up: 0, down: 0, comments: 0 };
    if (row.sentiment === 'approve') result[id].up++;
    else if (row.sentiment === 'disapprove') result[id].down++;
    if (row.comment && row.comment.trim().length > 0) result[id].comments++;
  }
